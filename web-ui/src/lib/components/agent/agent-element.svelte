...
<a
	...
	class="group w-64 h-64 mx-auto bg-surface-100-800-token border border-primary-500 rounded-lg shadow-lg transition transform hover:scale-105"
>
	<span
		class="text-primary-500 justify-center text-lg font-bold {hover} transition flex flex-row py-2 px-4"
	>
	...
	<div class="flex flex-col justify-center h-48">
		<div
			class="{agent.status.toString() === 'Online'
				...
				: 'text-primary-700'} justify-start text-sm transition flex flex-row py-2 px-4"
		>
			<Cable size="20" class="mr-1" />
			{agent.status}
		</div>
		<div class="text-primary-500 justify-start text-sm {hover} transition flex flex-row py-2 px-4">
